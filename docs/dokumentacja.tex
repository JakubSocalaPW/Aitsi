...
% ============================================================
\section{Integracja frontend--backend, walidacja i obsługa błędów}
% ============================================================

\subsection{Komunikacja HTTP}

Frontend komunikuje się z backendem przez bibliotekę \texttt{ky} --- minimalistyczny klient
HTTP oparty na Fetch API. Centralna instancja klienta tworzona jest w \texttt{api/client.ts}
i automatycznie dołącza nagłówek autoryzacyjny do każdego żądania:

\begin{lstlisting}[style=typescript, caption={Konfiguracja centralnego klienta HTTP (api/client.ts)}]
...
Każdy zasób API ma własny moduł (\texttt{photos.api.ts}, \texttt{categories.api.ts} itd.)
eksportujący funkcje z silnym typowaniem TypeScript --- odpowiedniki DTO z backendu.

\subsection{Zarządzanie stanem (Pinia)}

Dane pobrane z API trafiają do Pinia stores, skąd są reaktywnie dostępne dla komponentów:

\begin{lstlisting}[style=plain, caption={Przepływ danych: API -> Store -> Komponent}]
...
\subsection{Route guards (autoryzacja nawigacji)}

Plik \texttt{router/guards.ts} sprawdza meta-pola trasy przed każdą nawigacją:

\begin{lstlisting}[style=plain]
requiresAuth: true   -> uzytkownik musi byc zalogowany (JWT w localStorage)
requiresAdmin: true  -> rola uzytkownika musi byc 'admin'
guestOnly: true      -> zalogowany uzytkownik przekierowany do /przegladaj
\end{lstlisting}

Przy błędzie 401 z API użytkownik jest automatycznie wylogowywany i przekierowany
na stronę logowania.

\subsection{Walidacja danych}

\subsubsection{Frontend (przed wysłaniem żądania)}

\begin{table}[H]
  \centering
  \caption{Reguły walidacji po stronie frontendu}
  \begin{tabular}{@{}L{3cm}L{6cm}L{5cm}@{}}
    \toprule
    \textbf{Pole} & \textbf{Reguła} & \textbf{Komponent} \\
    \midrule
    Plik zdjęcia   & Wymagany; typy: jpg, png, webp; max 20 MB        & \texttt{ImageDropzone.vue} \\
    \addlinespace
    Tytuł          & Wymagany; max 200 znaków                          & \texttt{PhotoUploadForm.vue} \\
    \addlinespace
    Kategoria      & Wymagana (select)                                 & \texttt{CategorySelect.vue} \\
    \addlinespace
    Data           & Format \texttt{YYYY/YYYY-MM/YYYY-MM-DD}; zakres 1800-bieżący rok & \texttt{DatePrecisionPicker.vue} \\
    \addlinespace
    Opis           & Max 2000 znaków                                   & \texttt{maxlength} na textarea \\
    \addlinespace
    Cytat          & Max 1000 znaków                                   & \texttt{maxlength} na textarea \\
    \addlinespace
    Powód blokady  & Wymagany (dialog blokady)                         & \texttt{BlockUserDialog.vue} \\
    \bottomrule
  \end{tabular}
\end{table}

\subsubsection{Backend (po otrzymaniu żądania)}

\begin{itemize}
  \item Plik: sprawdzany przez \texttt{IFormFile} --- brak pliku zwraca \texttt{400 Bad Request}.
  \item Token OAuth: Google --- weryfikacja przez \texttt{googleapis.com/tokeninfo};
        Facebook --- weryfikacja przez \texttt{graph.facebook.com}.
  \item Autoryzacja: \texttt{[Authorize]} i \texttt{[Authorize(Roles = "admin")]}
        na metodach kontrolera.
  \item Własność zasobu: przy \texttt{PUT}/\texttt{DELETE} na zdjęciu sprawdzany
        \texttt{AuthorId == currentUserId} lub rola admin.
\end{itemize}

\subsection{Obsługa błędów}

\begin{lstlisting}[style=plain, caption={Hierarchia obsługi błędów}]
Blad sieciowy / timeout
    `-- ky rzuca HTTPError lub TypeError
           `-- catch w bloku try/catch w api/*.ts lub komponencie
                  `-- toastStore.show('Wystapil blad', 'error')
                         `-- AppToast.vue wyswietla komunikat (aria-live="assertive")

Blad 401 Unauthorized
    `-- uzytkownik wylogowany, redirect -> /logowanie

Blad 403 Forbidden
    `-- komunikat toast "Brak uprawnien"

Blad 404 Not Found
    `-- redirect -> NotFoundPage.vue
\end{lstlisting}

Przy próbie logowania zablokowanego konta backend zwraca JSON z polem \texttt{message}
(np. \texttt{"Konto zostało zablokowane. Powód: spam"}). Frontend odczytuje tę wiadomość
i wyświetla ją użytkownikowi zamiast generycznego komunikatu o błędzie.
...
\begin{lstlisting}[style=plain, caption={Mapa strony (sitemap)}]
/                           Strona glowna (publiczna)
+-- /przegladaj             Przegladaj archiwum (publiczna)
|   `-- /przegladaj/:id     Wyniki dla kategorii (publiczna)
+-- /zdjecie/:id            Szczegoly zdjecia (publiczna)
+-- /mapa                   Mapa interaktywna (publiczna)
+-- /kroniki                Kroniki spolecznosci / feed (publiczna)
+-- /logowanie              Logowanie OAuth (tylko goscie)
|
+-- /dodaj-zdjecie          Upload zdjecia (wymaga: Tworce)
+-- /moje-zdjecia           Moje materialy (wymaga: Tworce)
|   `-- /edytuj-zdjecie/:id Edycja zdjecia (wymaga: Tworce / Admin)
+-- /panel-tworcy           Dashboard tworcy (wymaga: Tworce)
|
`-- /admin                  Panel administracyjny (wymaga: Admin)
    +-- /admin/moderacja    Kolejka moderacji - nowe zdjecia (wymaga: Admin)
    +-- /admin/zdjecia      Wszystkie zdjecia zatwierdzone (wymaga: Admin)
    +-- /admin/uzytkownicy  Zarzadzanie uzytkownikami (wymaga: Admin)
    `-- /admin/kategorie    Zarzadzanie kategoriami (wymaga: Admin)
\end{lstlisting}

\textbf{Uzasadnienie przyjętej struktury URL:}
\begin{itemize}
  \item Hierarchia \texttt{/przegladaj} $\to$ \texttt{/przegladaj/:id} rozwiązuje problem
        orientacji — użytkownik wie, że jest „wewnątrz" archiwum, a kategoria jest podkontekstem.
  \item Prefix \texttt{/admin/*} izoluje strefy administracyjne i umożliwia jednolitą ochronę
        przez route guard (\texttt{requiresAdmin}).
  \item Polskie URL-e (\texttt{/przegladaj}, \texttt{/zdjecie}, \texttt{/moje-zdjecia}) są spójne
        z językiem interfejsu i poprawiają czytelność.
\end{itemize}

\subsection{Schemat organizacji treści}

\begin{lstlisting}[style=plain, caption={Schemat trzech osi klasyfikacji zdjęć}]
                +-----------------------------+
                |         ARCHIWUM            |
                |   (zbior wszystkich zdjec)  |
                +-------------+---------------+
                              |
         +--------------------+--------------------+
         |                    |                    |
...
+--------v--------------------v--------------------v-------+
|                         ZDJECIE                          |
|   tytul . opis . data . lokalizacja . tagi . metadane   |
+----------------------------------------------------------+
\end{lstlisting}

\begin{table}[H]
  \centering
  \caption{Problemy użytkownika a rozwiązania struktury informacji}
  \begin{tabular}{@{}L{6cm}L{8cm}@{}}
    \toprule
    \textbf{Problem} & \textbf{Rozwiązanie} \\
    \midrule
...
Panel administracyjny (/admin)
   +-- Moderacja zdjec (/admin/moderacja)
   |     +-- Lista nowych zdjec oczekujacych na akceptacje
   |     +-- Akceptuj -> zdjecie staje sie widoczne publicznie
   |     +-- Podglad -> /zdjecie/:id
   |     `-- Usun -> usuwa zdjecie i plik z dysku
   +-- Wszystkie zdjecia (/admin/zdjecia)
   |     +-- Przegladaj zatwierdzone materialy (paginacja, sortowanie)
   |     +-- Edytuj dowolne zdjecie -> /edytuj-zdjecie/:id
   |     `-- Usun dowolne zdjecie
   +-- Zarzadzaj uzytkownikami (/admin/uzytkownicy)
   |     `-- Blokuj / odblokuj uzytkownika (z podaniem powodu)
   `-- Zarzadzaj kategoriami (/admin/kategorie)
         +-- Dodaj kategorie
         +-- Edytuj kategorie
         `-- Usun kategorie (zablokowane jesli ma podkategorie lub zdjecia)
\end{lstlisting}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}L{3.8cm}L{2.2cm}L{2.3cm}L{5cm}@{}}
    ...
    \texttt{IsApproved}      & bool      & NOT NULL, default false & Czy zaakceptowane przez moderatora \\
...
Nowe zdjęcia trafiają do systemu z \texttt{IsApproved = false} i są widoczne wyłącznie
w panelu moderacji admina (\texttt{/admin/moderacja}). Po akceptacji (\texttt{IsApproved = true})
zdjęcie pojawia się w publicznym archiwum i wynikach wyszukiwania. Publiczny endpoint
\texttt{GET /api/photos} filtruje automatycznie tylko zatwierdzone materiały.
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}L{3.5cm}L{4.5cm}L{5.5cm}@{}}
    ...
    \texttt{categoryId}  & \texttt{CategoryId} + poddrzewo             & IN (zbiór ID, BFS) \\
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}L{1.5cm}L{4.5cm}L{2.5cm}L{5.5cm}@{}}
    ...
    \textbf{Met.} & \textbf{Endpoint} & \textbf{Dostęp} & \textbf{Opis} \\
    \midrule
    GET    & \texttt{/api/photos}        & \textbf{[PUBLICZNY]}         & Wyszukiwanie i paginacja zdjęć \textbf{zaakceptowanych} (\texttt{IsApproved = true}) \\
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}L{3cm}L{2cm}L{9cm}@{}}
    ...
    \texttt{categoryId} & int    & Filtrowanie po kategorii --- zwraca zdjęcia z wybranej kategorii \textbf{i wszystkich jej podkategorii} (BFS po drzewie) \\
...
\subsubsection{Body \texttt{PUT /api/photos/\{id\}} (\texttt{application/json})}

Wszystkie pola są opcjonalne --- przesyłane są wyłącznie zmieniające się wartości:

\begin{lstlisting}[style=plain, caption={Przykładowe ciało żądania PUT /api/photos/\{id\}}]
{
  "title":           "Nowy tytul",
  "description":     "Opis...",
  "categoryId":      3,
  ...
  "lat":             50.0614,
  "lng":             19.9372,
  "locationLabel":   "Krakow, Rynek Glowny",
  "technique":       "Sepia",
  "quote":           "Wspomnienie...",
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}L{1.5cm}L{5.5cm}L{2cm}L{5cm}@{}}
    ...
    GET & \texttt{/api/admin/stats}                        & \textbf{[ADMIN]} & Statystyki: \texttt{totalPhotos}, \texttt{totalUsers}, \texttt{blockedUsers}, \texttt{pendingPhotos} \\
    GET & \texttt{/api/admin/photos/pending}               & \textbf{[ADMIN]} & Paginowana lista zdjęć oczekujących na moderację (\texttt{IsApproved = false}) \\
    PUT & \texttt{/api/admin/photos/\{id\}/approve}        & \textbf{[ADMIN]} & Akceptacja zdjęcia --- ustawia \texttt{IsApproved = true} \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Przykładowe odpowiedzi}

\subsubsection{\texttt{GET /api/photos} --- odpowiedź stronicowana}

\begin{lstlisting}[style=plain, caption={Odpowiedź GET /api/photos}]
{
  "items": [
    {
      "id": 42,
      "title": "Rynek Glowny, 1965",
      "description": "Widok na fontanne z Neptunem...",
      "imagePath": "/uploads/abc123.jpg",
      "thumbnailPath": "/uploads/abc123_thumb.jpg",
      "lat": 50.0614,
      "lng": 19.9372,
      "locationLabel": "Krakow, Rynek Glowny",
      "date": "1965",
...
\subsubsection{\texttt{POST /api/auth/google} --- odpowiedź po zalogowaniu}

\begin{lstlisting}[style=plain, caption={Odpowiedź POST /api/auth/google}]
{
...
Pola \texttt{hasGoogle} i \texttt{hasFacebook} informują frontend, przez jakich dostawców
użytkownik ma powiązane konto, nie ujawniając surowych identyfikatorów OAuth.
...
\begin{longtable}{@{}L{1.3cm}L{6.2cm}L{1.6cm}L{1.6cm}L{1.6cm}@{}}
  ...
  GET    & \texttt{/api/admin/photos/pending}            & & & \checkmark \\
  PUT    & \texttt{/api/admin/photos/\{id\}/approve}     & & & \checkmark \\
\end{longtable}

\textbf{Łącznie: 24 endpointy} — 7~publicznych, 6~wyłącznie dla twórcy, 11~wyłącznie dla admina.
...
\subsubsection{1.4.13 — Zawartość przy najechaniu / fokusie (poziom AA)}

Powiadomienia toast (\texttt{AppToast}) wyświetlają się czasowo i nie blokują interakcji.
Nie są stosowane niestandardowe tooltip-y mogące przesłaniać treść.
...
\subsubsection{2.4.4 — Cel linku (poziom A)}

Linki paginacji mają \texttt{aria-label} z numerem strony. Linki nawigacji mają
\texttt{aria-current="page"} na aktywnej pozycji.

\subsubsection{2.4.6 — Nagłówki i etykiety (poziom AA)}

Każda strona posiada jeden nagłówek \texttt{<h1>}. Hierarchia
\texttt{h1} $\to$ \texttt{h2} $\to$ \texttt{h3} jest zachowana. Sekcje filtrów
i wyników posiadają nagłówki.

\subsubsection{2.3.3 — Animacja z interakcji (poziom AAA --- zrealizowane)}

Reguła CSS \texttt{@media (prefers-reduced-motion: reduce)} wyłącza animacje
przejść między stronami i \texttt{scroll-behavior: smooth}.
...
\subsubsection{3.3.2 — Etykiety lub instrukcje (poziom A)}

Każde pole formularza posiada widoczną etykietę. Pola opcjonalne i wymagane oznaczone
są gwiazdką \texttt{*} z wyjaśnieniem w kontekście formularza. Pole tagów zawiera
wskazówkę „Oddziel tagi przecinkami".
...
\subsection{Dostosowanie wyglądu}

System implementuje pełną trójkę wariantów wyglądu:

\begin{table}[H]
  \centering
  \caption{Motywy wizualne i skale czcionki}
  \begin{tabular}{@{}L{3.5cm}L{5cm}L{5.5cm}@{}}
    ...
    \texttt{light}         & Domyślny lub \texttt{prefers-color-scheme: light} & Standardowe użytkowanie \\
    \addlinespace
    \texttt{dark}          & \texttt{prefers-color-scheme: dark} lub ręczny    & Komfort w słabym oświetleniu \\
    \addlinespace
    \texttt{high-contrast} & \texttt{prefers-contrast: more} lub ręczny        & Osoby słabowidzące \\
    \bottomrule
  \end{tabular}
\end{table}

Wybór motywu persystowany w \texttt{localStorage} (klucz \texttt{cas\_theme}). Zmiana
następuje natychmiastowo przez atrybut \texttt{data-theme} na \texttt{<html>} bez
przeładowania strony.

Skala czcionki persystowana osobno (klucz \texttt{cas\_fontsize}):
\begin{itemize}
  \item \texttt{small} --- baza 0.875 rem,
  \item \texttt{normal} --- baza 1 rem,
  \item \texttt{large} --- baza 1.125 rem.
\end{itemize}
